...
export default function ScanningPage() {
  ...
  useEffect(() => {
      ...
      if (userEmail) {
        const fetchHistory = async () => {
          try {
            ...
            const res = await fetch(`http://${window.location.hostname}:4000/api/scan/history?userEmail=${encodeURIComponent(userEmail)}`, { headers });
    ...
    const startScan = async () => {
      try {
        const res = await fetch(`http://${window.location.hostname}:4000/api/scan/start`, {
          method: 'POST',
        ...
        // Open SSE connection
        eventSource = new EventSource(`http://${window.location.hostname}:4000/api/scan/progress?scanId=${newScanId}`);
